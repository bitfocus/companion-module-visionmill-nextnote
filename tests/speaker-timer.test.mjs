...
function snapshot(sequence = 1, session = 'show-a') {
	return {
		...
		clock: '19:30:00',
		speaker: 'Clare',
		outputs: Object.fromEntries(timerOutputs.map((k) => [k, { enabled: true, mode: 'countdown' }])),
	}
}
test('timer feedback validates the complete native contract', () => {
	...
	for (const invalid of [
		null,
		'{',
		JSON.stringify({ ...snapshot(), duration: 0 }),
		JSON.stringify({ ...snapshot(), remaining: NaN }),
		JSON.stringify({ ...snapshot(), outputs: {} }),
		JSON.stringify({ ...snapshot(), color: 'red' }),
	])
		assert.equal(parseTimerSnapshot(invalid), null)
})
test('reordered feedback and retired app sessions cannot roll state back', () => {
	const values = []
	const instance = { setVariableValues: (v) => values.push(v), checkFeedbacks() {}, sendOSC() {} }
...
test('stale feedback clears live values and requests authoritative state', (t) => {
	...
	const timer = new SpeakerTimerCompanion({
		setVariableValues: (v) => values.push(v),
		checkFeedbacks() {},
		sendOSC: (c) => commands.push(c),
	})
	timer.start()
	timer.receive(JSON.stringify(snapshot()))
	now = 3000
...
test('duration, live adjustment and output commands use the common bounded paths', () => {
	const sent = [],
		actions = timerActions({ sendOSC: (address) => sent.push(address) })
	...
	assert.deepEqual(
		actions.timer_mode.options[0].choices.map((choice) => choice.id),
		['presenter', 'next', 'prompter'],
	)
	assert.deepEqual(sent, [
		'/nextnote/timer/set-start/900',
		'/nextnote/timer/adjust/-30',
		'/nextnote/timer/visibility/web/0',
		'/nextnote/timer/mode/prompter/both',
	])
	const presets = timerPresets()
	for (const preset of Object.values(presets))
		for (const step of preset.steps) for (const action of step.down) assert.ok(actions[action.actionId])
})
test('flash supports toggle, explicit on/off and fresh selected feedback', () => {
	const sent = [],
		values = []
	const instance = {
		sendOSC: (address) => sent.push(address),
		setVariableValues: (value) => values.push(value),
		checkFeedbacks() {},
	}
	instance.speakerTimer = new SpeakerTimerCompanion(instance)
	const actions = timerActions(instance),
		feedbacks = timerFeedbacks(instance)
...
test('timer presets are discoverable without a Helper and send the intended native commands', () => {
	...
	const { structure, presets } = getPresets(instance),
		actions = timerActions(instance)
	const groups = structure
		.flatMap((section) => section.definitions)
		.filter((group) => group.id.startsWith('speaker_timer'))
	...
	for (const id of ids) {
		...
		for (const step of presets[id].steps)
			for (const action of step.down) assert.ok(actions[action.actionId], `Preset ${id} references a real action`)
	}
	for (const [id, path] of [
		['timer_reset', 'reset'],
		['timer_flash', 'flash-toggle'],
		['timer_start', 'start'],
		['timer_pause', 'pause'],
		['timer_adjust_-300', 'adjust/-300'],
		['timer_adjust_-60', 'adjust/-60'],
		['timer_adjust_-30', 'adjust/-30'],
		['timer_adjust_30', 'adjust/30'],
		['timer_adjust_60', 'adjust/60'],
		['timer_adjust_300', 'adjust/300'],
		['timer_set_custom', 'set/900'],
		['timer_undo', 'undo'],
	]) {
...
test('dedicated transport pulses only while its state is active and resets to steady grey', (t) => {
	...
	for (const [index, state] of ['ready', 'running', 'overtime', 'paused', 'finished', 'ready'].entries()) {
		...
		for (const control of ['start', 'pause']) {
			...
			const bright = timerTransportColour(s, true, control, 0),
				dim = timerTransportColour(s, true, control, 500)
		...
		assert.equal(
			calls.some((ids) => ids.includes('timer_transport')),
			['running', 'overtime', 'paused'].includes(state),
		)
	}
	timer.stop()
	calls.length = 0
...
test('separate time digits are padded, signed once and cleared when stale', () => {
	let values
	const timer = new SpeakerTimerCompanion({ setVariableValues: (v) => (values = v), checkFeedbacks() {}, sendOSC() {} })
	...
	timer.start()
	assert.deepEqual([values.timer_hours, values.timer_minutes, values.timer_seconds], ['—', '—', '—'])
	timer.stop()
...
test('global visibility preset sends toggle and follows native feedback', () => {
	const sent = []
	const instance = {
		sendOSC: (path) => sent.push(path),
		speakerTimer: { fresh: true, snapshot: { ...snapshot(), visible: false } },
	}
	...
	instance.speakerTimer.snapshot.visible = true
	assert.equal(timerFeedbacks(instance).timer_visible.callback(), true)
...
test('Set accepts hours/minutes/seconds and minutes/seconds, rejecting malformed input', () => {
	const sent = []
	const actions = timerActions({ sendOSC: (path) => sent.push(path) })
	for (const [text, seconds] of [
		...
		actions.timer_set.callback({ options: { seconds: text } })
		assert.equal(sent.at(-1), `/nextnote/timer/set/${seconds}`)
		actions['timer_set-start'].callback({ options: { seconds: text } })
		assert.equal(sent.at(-1), `/nextnote/timer/set-start/${seconds}`)
	}
	const count = sent.length
	for (const text of ['1:60:00', '10:60', '0:00', '', 'abc', '1:2:3:4', '-1:00'])
		actions.timer_set.callback({ options: { seconds: text } })
